...
CURRENT_SOURCE_POS = (0, 3)
TARGET_POS = (5, 5)
CURRENT = 2  # Amps
REQUIRED_FORCE = 144  # N
MAX_PATH_LENGTH = 8  # Optional 
GRID_START_X = 120  # horizontal offset from left
GRID_START_Y = 180   # vertical offset from top
...
def draw_grid():
    ...
    for y in range(GRID_HEIGHT):
        for x in range(GRID_WIDTH):
            rect = pygame.Rect(GRID_START_X + x * TILE_SIZE, GRID_START_Y + y * TILE_SIZE, TILE_SIZE, TILE_SIZE)
            ...
            if (x, y) == CURRENT_SOURCE_POS:
                ...
                draw_text("I = 2A", GRID_START_X + x * TILE_SIZE + 5, GRID_START_Y + y * TILE_SIZE + 15)
    ...
    draw_text(f"Max Steps Allowed = {MAX_PATH_LENGTH} (Length)", 10, 10)
    ...
    draw_text(f"Calculate B needed and find the path from the ", 10, 60)
    draw_text(f"source where the tiles add up to the value", 10, 80)
